...
#define DEBUG_LOG_TAG "PERFHELPER"
    ...
    , m_fn_perf_lock_rel(nullptr)
{
    ...
    {
        HWC_LOGE("failed to open library[%s]", LIBRARY_NAME);
    }
}
...
int PerfHelper::loadApi()
{
    ...
    if (m_so_handle != nullptr)
    {
        ...
        {
            HWC_LOGW("failed to find perf_lock_acq");
            res = -EPERM;
        }
        if (m_fn_perf_lock_rel == nullptr)
        {
            HWC_LOGW("failed to find perf_lock_rel");
            res = -EPERM;
        }
    }
    else
