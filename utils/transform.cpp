...
#define DEBUG_LOG_TAG "TRANSFORM"
...
Rect Transform::transform(const Rect& bounds, bool roundOutwards) const
{
    ...
    if (roundOutwards) {
        r.left   = static_cast<int32_t>(floorf(min(lt[0], rt[0], lb[0], rb[0])));
        r.top    = static_cast<int32_t>(floorf(min(lt[1], rt[1], lb[1], rb[1])));
        r.right  = static_cast<int32_t>(ceilf(max(lt[0], rt[0], lb[0], rb[0])));
        r.bottom = static_cast<int32_t>(ceilf(max(lt[1], rt[1], lb[1], rb[1])));
    } else {
        r.left   = static_cast<int32_t>(floorf(min(lt[0], rt[0], lb[0], rb[0]) + 0.5f));
        r.top    = static_cast<int32_t>(floorf(min(lt[1], rt[1], lb[1], rb[1]) + 0.5f));
        r.right  = static_cast<int32_t>(floorf(max(lt[0], rt[0], lb[0], rb[0]) + 0.5f));
        r.bottom = static_cast<int32_t>(floorf(max(lt[1], rt[1], lb[1], rb[1]) + 0.5f));
    }
...
Transform Transform::inverse() const {
    ...
    if (mType <= TRANSLATE) {
        ...
        const float idet = static_cast<float>(1.0 / (a*d - b*c));
...
void Transform::dump(const char* name) const
{
    ...
    HWC_LOGD("%s 0x%08x (%s, %s)", name, mType, flags.string(), type.string());
    HWC_LOGD("%.4f  %.4f  %.4f", m[0][0], m[1][0], m[2][0]);
    HWC_LOGD("%.4f  %.4f  %.4f", m[0][1], m[1][1], m[2][1]);
    HWC_LOGD("%.4f  %.4f  %.4f", m[0][2], m[1][2], m[2][2]);
}
