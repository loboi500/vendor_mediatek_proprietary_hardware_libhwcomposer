...
bool PlatformCommon::isUILayerValid(const sp<HWCLayer>& layer, int32_t* line)
{
    ...
    if (!HWCMediator::getInstance().getOvlDevice(HWC_DISPLAY_PRIMARY)->isDisplaySupportedWidthAndHeight(priv_hnd.width, priv_hnd.height))
    {
        *line = __LINE__;
        return false;
    }
        ...
        int32_t* line)
{
    ...
    setPQEnhance(disp->getId(), layer->getPrivateHandle(), &pq_enhance, is_game, is_camera_preview_hdr);
    ...
    uint32_t out_format = layer->decideMdpOutputFormat();
    uint32_t convert_format = 0;

    if (out_format == 0)
    {
        out_format = priv_hnd.format;
    }
    ...
    convert_format = convertFormat4Bliter(out_format);

    bool is_blit_valid =
        queryHWSupport(disp->getId(),
...
bool PlatformCommon::queryHWSupport(const uint64_t& dpy, uint32_t srcWidth, uint32_t srcHeight,
...
bool PlatformCommon::isGlaiLayerValid(const sp<HWCLayer>& layer, int32_t* line)
{
    ...
    if (!HWCMediator::getInstance().getOvlDevice(HWC_DISPLAY_PRIMARY)->isDisplaySupportedWidthAndHeight(out_w, out_h))
    {
        *line = __LINE__;
        ...
        HEIGHT(layer->getDisplayFrame()) != HEIGHT(out_dst_roi))
    {
        ...
        else
        {
            ...
            if (src_width > HWCMediator::getInstance().getOvlDevice(HWC_DISPLAY_PRIMARY)->getRszMaxWidthInput() &&
                (out_fmt == HAL_PIXEL_FORMAT_RGBX_8888 ||
...
bool PlatformCommon::isMMLPrimarySupport()
{
    return (HwcFeatureList::getInstance().getFeature().is_support_mml &&
           Platform::getInstance().m_config.mml_switch &&
           HWCMediator::getInstance().getOvlDevice(HWC_DISPLAY_PRIMARY)->isMMLPrimarySupported());
}
...
PlatformCommon::PlatformConfig::PlatformConfig()
    ...
    , is_support_sf_present_fence(false)
