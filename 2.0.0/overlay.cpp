...
OverlayEngine::OverlayEngine(const uint64_t& dpy, uint32_t width, uint32_t height)
    : m_disp_id(dpy)
    , m_handling_job(NULL)
    ...
    , m_power_mode_changed(0)
{
    ...
    status_t err = HWCMediator::getInstance().getOvlDevice(m_disp_id)->createOverlaySession(m_disp_id, width, height);
    ...
    else
    {
        ...
        unsigned int avail_inputs = m_max_inputs;
...
void OverlayEngine::trigger(const unsigned int& present_fence_idx,
                            const unsigned int& sf_present_fence_idx,
                            ...
                            const bool& do_nothing)
{
    if (!do_nothing)
    {
        ...
        if (frame_info)
        {
            ...
            packageFrameInfo(frame_info, present_fence_idx, sf_present_fence_idx,
    ...
    else
    {
        ...
        logger.printf("(%" PRIu64 ") Trigger with idx: %u/%u/ do_nothing", m_disp_id,
                      present_fence_idx, sf_present_fence_idx);
    }
...
status_t OverlayEngine::loopHandler(sp<FrameInfo>& info)
{
    ...
    int sf_fence_idx = info->sf_present_fence_idx;
    ...
    {
        DbgLogger logger(DbgLogger::TYPE_HWC_LOG, 'D', nullptr);
        logger.printf("(%" PRIu64 ") triggerOverlaySession ovlp:%d idx:%d sf_idx:%d prev_present_fence:%d",
            m_disp_id, ovlp_layer_num, fence_idx, sf_fence_idx, prev_present_fence);
    }
    ...
    err = HWCMediator::getInstance().getOvlDevice(m_disp_id)->triggerOverlaySession(
        m_disp_id, fence_idx, sf_fence_idx, ovlp_layer_num, prev_present_fence, config, hrt_weight, hrt_idx,
...
{
    DbgLogger logger(DbgLogger::TYPE_HWC_LOG, 'D', nullptr);
    logger.printf("(%" PRIu64 ") Trigger with idx: %u/%u", m_disp_id, present_fence_idx, sf_present_fence_idx);
    ...
    info->sf_present_fence_idx = static_cast<int>(sf_present_fence_idx);
    ...
    for (unsigned int i = 0; i < m_handling_job->num_layers; i++)
    {
        ...
        layer->fb_id = 0;
        logger.printf("/ %u,s:%d,f_fd:%d,ion:%d", i, layer->state, layer->fence, layer->ion_fd);
    }
    ...
    if (HWCMediator::getInstance().getLowLatencyWFD() == true)
    {
        info->skip_config = m_handling_job->is_same_dpy_content;
    }
    else
    {
        info->skip_config = WFD_INV_VAL;
    }
