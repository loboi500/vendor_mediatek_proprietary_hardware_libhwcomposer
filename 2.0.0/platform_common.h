...
public:
    ...
    // check is support mml primary
    bool isMMLPrimarySupport();
    ...
    struct PlatformConfig
    {
        ...
        // The layer with hint_id will use hint_hwlayer_type as
        // its composition type as much as possible (only enable when hint_id > 0)
        uint64_t hint_id;
        ...
        // seperate present fence of sf which will align TE to avoid resync incorrectly
        // this case is usally used in VDO mode
        bool is_support_sf_present_fence;
        ...
        // this value is for MML UT test use.
        bool test_mml;
...
private:
    static bool queryHWSupport(const uint64_t& dpy,
                                uint32_t srcWidth,
                                uint32_t srcHeight,
                                uint32_t dstWidth,
                                uint32_t dstHeight,
