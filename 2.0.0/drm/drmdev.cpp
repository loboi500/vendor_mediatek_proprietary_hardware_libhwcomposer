...
FrameCfg::FrameCfg()
    : session_id(MTK_DRM_INVALID_SESSION_ID)
    , mode(MTK_DRM_SESSION_INVALID)
{
}
    ...
    : m_drm(&DrmModeResource::getInstance())
{
    for (unsigned int i = 0; i < DisplayManager::MAX_DISPLAYS; i++)
    {
        m_frame_cfg[i].session_id = MTK_DRM_INVALID_SESSION_ID;
        m_frame_cfg[i].mode = MTK_DRM_SESSION_INVALID;
    }
    ...
    m_max_overlay_num = static_cast<unsigned int>(m_drm->getDisplay(HWC_DISPLAY_PRIMARY)->getPlaneNum());
...
void DrmDevice::queryCapsInfo()
{
    ...
    HWC_LOGD("CapsInfo [%d] MMLPrimarySupported", isMMLPrimarySupported());
...
uint32_t DrmDevice::getDisplayRotation(uint64_t dpy)
{
    uint32_t rotation = 0;
    if (HWC_DISPLAY_PRIMARY == dpy)
    {
        if (MTK_LCM_DEGREE_180 == m_caps_info.lcm_degree)
        {
...
bool DrmDevice::isMMLPrimarySupported()
{
    return m_caps_info.disp_feature_flag & DRM_DISP_FEATURE_MML_PRIMARY;
}
...
status_t DrmDevice::createOverlaySession(uint64_t dpy, uint32_t width, uint32_t height,
                                         HWC_DISP_MODE mode)
{
    CHECK_DPY_RET_STATUS(dpy);
    ...
    if (MTK_DRM_INVALID_SESSION_ID != session_id)
    {
    ...
    if (err < 0)
    {
        m_frame_cfg[dpy].session_id = MTK_DRM_INVALID_SESSION_ID;
        m_frame_cfg[dpy].mode = MTK_DRM_SESSION_INVALID;
    ...
    DrmModeCrtc *crtc = m_drm->getDisplay(static_cast<uint32_t>(dpy));
    crtc->setReqSize(width, height);

    DLOGI(dpy, "Create Session (%s)", getSessionModeString(mode).string());

    return NO_ERROR;
}

void DrmDevice::destroyOverlaySession(uint64_t dpy)
{
    ...
    err = m_drm->blankDisplay(dpy, HWC_POWER_MODE_OFF);
    if (err)
    {
...
status_t DrmDevice::triggerOverlaySession(uint64_t dpy, int present_fence_idx, int sf_present_fence_idx,
                                          ...
                                          )
{
    ...
    HWC_ATRACE_FORMAT_NAME("TrigerOVL:%d sf:%d", present_fence_idx, sf_present_fence_idx);
    ...
    if (m_atomic_req[dpy] != nullptr)
    {
        ...
        else
        {
            ...
            ret |= crtc->addProperty(m_atomic_req[dpy], DRM_PROP_CRTC_SF_PRESENT_FENCE,
                         static_cast<uint64_t>(sf_present_fence_idx)) < 0;
        }
        ...
        if (DisplayManager::m_profile_level & PROFILE_DBG_WFD)
        {
            ret |= crtc->addProperty(m_atomic_req[dpy], DRM_PROP_CRTC_OVL_DSI_SEQ,
                              static_cast<uint64_t>(trigger_param.ovl_seq % UINT32_MAX));
        }

        if (dpy == HWC_DISPLAY_VIRTUAL &&
                trigger_param.skip_config != WFD_INV_VAL)
        {
            ret |= crtc->addProperty(m_atomic_req[dpy],DRM_PROP_CRTC_SKIP_CONFIG,
                                        static_cast<uint64_t>(trigger_param.skip_config)) < 0;
        }

        ret = m_drm->atomicCommit(m_atomic_req[dpy], flags, nullptr);
        if (ret)
        {
            HWC_LOGE("(%" PRIu64 ") failed to drmModeAtomicCommit: ret=%d ovlp:%d pf_idx:%d sf_pf_idx:%d hrt_idx:%d mode:%d",
                     dpy, ret, ovlp_layer_num, present_fence_idx, sf_present_fence_idx, hrt_idx, config);
        }
        else
        {
            logger.printf("sid:%d ovlp:%d pf_idx:%d sf_pf_idx:%d hrt_idx:%d mode:%d",
                    m_frame_cfg[dpy].session_id, ovlp_layer_num, present_fence_idx,
                    sf_present_fence_idx, hrt_idx, config);
        }
    ...
    uint64_t dpy, OverlayPortParam* const* /*params*/, unsigned int /*num*/)
{
    ...
    DrmModeCrtc *crtc = m_drm->getDisplay(dpy);
    if (!crtc) {
        HWC_LOGE("(%" PRIu64 ") drm state is wrong when disableOverlaySession", dpy);
        return;
    }
    ...
    triggerOverlaySession(dpy, 0, 0, 0, -1, 0, 0, 0, 0, nullptr, nullptr, {});
...
status_t DrmDevice::setOverlaySessionMode(uint64_t dpy, HWC_DISP_MODE mode)
{
    ...
    int err = 0;
    if (res != INVALID_OPERATION && (m_frame_cfg[dpy].mode != session_mode ||
            m_display_state.getNumberWaitedDisplay() != 0))
    {
        ...
        {
            m_frame_cfg[dpy].mode = session_mode;
        }
        ...
        if (m_display_state.getNumberWaitedDisplay())
        {
            for (unsigned int i = 0; i < DisplayManager::MAX_DISPLAYS; i++)
            {
                ...
                {
                    int ret = m_drm->setDisplay(i, (i == HWC_DISPLAY_VIRTUAL) ? true : false);
                    if (ret)
                    {
                        DLOGE(dpy, "setDisplay_%d fail, ret %d", i, ret);
                        abort();
                    }
...
HWC_DISP_MODE DrmDevice::getOverlaySessionMode(uint64_t dpy)
{
...
status_t DrmDevice::getOverlaySessionInfo(uint64_t dpy, SessionInfo* info)
{
    CHECK_DPY_RET_STATUS(dpy);
    ...
    {
        HWC_LOGW("(%" PRIu64 ") Failed to get invalid session info", dpy);
        return INVALID_OPERATION;
    }
    ...
    if (err < 0)
    {
        IOLOGE(dpy, err, "DRM_IOCTL_MTK_GET_SESSION_INFO");
    }

    DrmModeCrtc *crtc = m_drm->getDisplay(static_cast<uint32_t>(dpy));
    if (!crtc)
    {
        HWC_LOGE("(%" PRIu64 ") drm state is wrong when getOverlaySessionInfo", dpy);
        return -ENODEV;
    }
...
unsigned int DrmDevice::getAvailableOverlayInput(uint64_t dpy)
{
    DrmModeCrtc *crtc = m_drm->getDisplay(static_cast<uint32_t>(dpy));
    if (!crtc)
    {
        HWC_LOGE("(%" PRIu64 "), %s(), drm state is wrong", dpy, __FUNCTION__);
        return 1;
    }
    ...
    uint64_t dpy, OverlayPrepareParam* param)
{
    ...
    if (err < 0)
    {
        IOLOGE(dpy, err, "DRM_IOCTL_MTK_GEM_SUBMIT");
    }
...
void DrmDevice::updateOverlayInputs(
...
void DrmDevice::disableOverlayOutput(uint64_t dpy)
{
    CHECK_DPY_RET_VOID(dpy);
...
void DrmDevice::enableOverlayOutput(uint64_t dpy, OverlayPortParam* param)
{
    CHECK_DPY_RET_VOID(dpy);
...
void DrmDevice::prepareOverlayPresentFence(uint64_t dpy, OverlayPrepareParam* param)
{
    ...
    if (!crtc)
    {
        HWC_LOGE("(%" PRIu64 ") drm state is wrong when prepareOverlayPresentFence", dpy);
        return;
    }
    ...
    if (res < 0)
    {
        IOLOGE(dpy, res, "DRM_IOCTL_MTK_CRTC_GETFENCE");
        param->fence_index = 0;
        param->fence_fd = -1;
    }
    else
    ...
    param->sf_fence_fd = -1;
    bool has_sf_present_fence_feature = m_caps_info.disp_feature_flag & DRM_DISP_FEATURE_SF_PRESENT_FENCE;
    if (dpy == HWC_DISPLAY_PRIMARY && Platform::getInstance().m_config.is_support_sf_present_fence)
    {
        if (has_sf_present_fence_feature)
        {
            int res = DRM_WDT_IOCTL(DRM_IOCTL_MTK_CRTC_GETSFFENCE, &fence_req);
            if (res < 0)
            {
                IOLOGE(dpy, res, "DRM_IOCTL_MTK_CRTC_GETSFFENCE");
            }
            else
            {
                param->is_sf_fence_support = true;
                param->sf_fence_index = fence_req.fence_idx;
                param->sf_fence_fd = fence_req.fence_fd;
            }
        }
    }
    HWC_LOGV("(%" PRIu64 ") DrmDevice::prepareOverlayPresentFence() crtc_id:%d fence_index:%d fence_fd:%d"
             "sf_fence_index:%d sf_fence_fd:%d sf_fence_support:%d feature:%d",
             dpy, fence_req.crtc_id, param->fence_index, param->fence_fd,
             param->sf_fence_index, param->sf_fence_fd, param->is_sf_fence_support, has_sf_present_fence_feature);
}

status_t DrmDevice::waitVSync(uint64_t dpy, nsecs_t* ts)
{
    CHECK_DPY_RET_STATUS(dpy);

    int err = m_drm->waitNextVsync(dpy, ts);
    if (err < 0)
    {
        HWC_LOGI("(%" PRIu64 ") failed to waitNextVsync: %d", dpy, err);
        return BAD_VALUE;
    }
    ...
    switch (mode)
    {
        ...
        case HWC_POWER_MODE_DOZE:
        {
            ...
            else
            {
                ...
                err = m_drm->blankDisplay(dpy, HWC_POWER_MODE_NORMAL);
                HWC_LOGE("(%" PRIu64 ") setPowerMode: receive HWC_POWER_MODE_DOZE without aod enabled",
                         dpy);
            }
            break;
        }
        case HWC_POWER_MODE_DOZE_SUSPEND:
        {
            ...
            else
            {
                // the project does not support AOD, so we keep it in normal mode
                err = m_drm->blankDisplay(dpy, HWC_POWER_MODE_NORMAL);
                HWC_LOGE("(%" PRIu64 ") setPowerMode: receive HWC_POWER_MODE_DOZE_SUSPEND without aod enabled",
                         dpy);
            }
            break;
        }
...
uint32_t DrmDevice::getDrmSessionMode(uint64_t dpy)
{
    if (!CHECK_DPY_VALID(dpy))
    {
        HWC_LOGE("%s(), invalid dpy %" PRIu64 "", __FUNCTION__, dpy);
...
uint32_t DrmDevice::getHrtIndex(uint64_t dpy)
{
    uint32_t pos = 0xffff;
    DrmModeCrtc *crtc = m_drm->getDisplay(static_cast<uint32_t>(dpy));
    if (crtc != nullptr)
    ...
    {
        HWC_LOGW("(%" PRIu64 ") failed to get HRT index, crtc is null", dpy);
    }
    return pos;
}
...
void DrmDevice::createFbId(OverlayPortParam* param, const uint64_t& dpy, const uint64_t& id)
{
    ...
    if (param->secure && (!isSupportDmaBuf() || HwcFeatureList::getInstance().getFeature().svp_on_mtee == 2))
    {
...
int32_t DrmDevice::getWidth(uint64_t dpy, hwc2_config_t config)
{
    return m_drm->getWidth(dpy, config);
}

int32_t DrmDevice::getHeight(uint64_t dpy, hwc2_config_t config)
{
    return m_drm->getHeight(dpy, config);
}

int32_t DrmDevice::getRefresh(uint64_t dpy, hwc2_config_t config)
{
    return m_drm->getRefresh(dpy, config);
}

uint32_t DrmDevice::getNumConfigs(uint64_t dpy)
{
    return m_drm->getNumConfigs(dpy);
}
...
status_t DrmDevice::createColorTransformBlob(const uint64_t& dpy, sp<ColorTransform> color_transform, uint32_t* id)
{
    ...
    if (color_transform != nullptr && id != nullptr)
    {
        ...
        {
            config.color_matrix[i] = transFloatToIntForColorMatrix(color_transform->matrix[i / 4][i % 4]);
        }
...
{
    return m_drm->updateCrtcToPreferredModeInfo(dpy);
}

int32_t DrmDevice::getCurrentRefresh(uint64_t dpy)
{
    return m_drm->getCurrentRefresh(dpy);
}
...
uint32_t DrmDevice::getDisplayUniqueId(uint64_t dpy)
{
