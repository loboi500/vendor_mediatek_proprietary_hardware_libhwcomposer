...
// ---------------------------------------------------------------------------
Platform_MT6785::Platform_MT6785()
{
    ...
    if (strstr("mt6785", TARGET_BOARD_PLATFORM))
    {
    ...
    else
    {
        ...
        m_config.mdp_support_decompress = false;
    }

    if (strstr("mt6781", TARGET_BOARD_PLATFORM))
    {
        m_config.plat_switch |= HWC_PLAT_SWITCH_NOTIFY_HWBINDER_TID;
    }
}

size_t Platform_MT6785::getLimitedExternalDisplaySize()
{
    // 4k resolution
    return 3840 * 2160;
}

bool Platform_MT6785::isUILayerValid(const sp<HWCLayer>& layer, int32_t* line)
{
...
bool Platform_MT6785::isMMLayerValid(const sp<HWCLayer>& layer, const int32_t pq_mode_id,
        int32_t* line)
{
    const int src_width = getSrcWidth(layer);
    const int src_height = getSrcHeight(layer);
