#define DEBUG_LOG_TAG "drmmodeplane"
...
int DrmModePlane::init(int fd)
{
    ...
    if (res)
    {
        ALOGE("failed to init plane[%d] property: errno[%d]", m_id, res);
        return res;
    }
    ...
    if (res)
    {
        ALOGE("failed to check plane[%d] property: error=%d", m_id, res);
        return res;
    }
...
{
    ALOGI("DrmModePlane: id=%d crtc_id=%d possible_crtc=%08x",
...
int DrmModePlane::connectCrtc(DrmModeCrtc *crtc)
{
    ...
    if (crtc == nullptr)
    {
        ALOGW("try to connect plane_%d with nullptr crtc", m_id);
        return res;
    }
