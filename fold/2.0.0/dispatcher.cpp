...
void DispatcherJob::resetData()
{
    ...
    drm_id_cur_connector = UINT32_MAX;
...
int HWCDispatcher::getJob(uint64_t dpy, hwc2_config_t active_config)
{
    ...
    int dispatcher_job_status = HWC_DISPACHERJOB_VALID;
    {
        ...
        if ((DisplayManager::getInstance().getDisplayData(dpy)->trigger_by_vsync ||
             (m_workers[dpy].is_internal && !m_workers[dpy].is_main)) &&
            m_workers[dpy].enable)
        {
            const nsecs_t cur_time = systemTime();
            const nsecs_t period = DisplayManager::getInstance().getDisplayData(dpy, active_config)->refresh;
            bool should_drop_job = (cur_time - m_prev_createjob_time[dpy]) <
                                   (period - Platform::getInstance().m_config.tolerance_time_to_refresh);
            size_t queue_size = isNoDispatchThread() ?
            ...
            if (should_drop_job || queue_size > 2)
            {
                ...
                {
                    m_workers[dpy].idle_thread->updatePeriod(period);
...
void HWCDispatcher::onPlugIn(uint64_t dpy, uint32_t drm_id_crtc, uint32_t drm_id_connector,
                             bool is_internal, bool is_main, uint32_t width, uint32_t height)
{
#ifndef MTK_USER_BUILD
    HWC_ATRACE_CALL();
#endif
...

    {
        ...
        if (!DisplayManager::getInstance().getDisplayData(dpy)->trigger_by_vsync)
        {
            ...
            if (is_internal && !m_workers[dpy].is_main)
            {
...
void HWCDispatcher::setPowerMode(uint64_t dpy, uint32_t drm_id_crtc, int mode, bool panel_stay_on)
{
#ifndef MTK_USER_BUILD
    HWC_ATRACE_CALL();
#endif
    ...
    if (dpy != HWC_DISPLAY_VIRTUAL)
    {
        ...
        if (m_workers[dpy].enable)
        {
            m_workers[dpy].ovl_engine->setPowerMode(drm_id_crtc, mode, panel_stay_on);
        }
...
void HWCDispatcher::updateDrmIdCurCrtc(uint64_t dpy, uint32_t drm_id_crtc)
{
    AutoMutex l(m_workers[dpy].plug_lock_main);

    m_workers[dpy].drm_id_cur_crtc = drm_id_crtc;
    m_workers[dpy].ovl_engine->updateDrmIdCurCrtc(drm_id_crtc);
}
...
void DispatchThread::calculatePerf(DispatcherJob* job)
{
    ...
    const nsecs_t cur_time = systemTime();
    // at least leave some time for OverlayEngine
    nsecs_t remain_time = job->decouple_target_ts <= 0 ? job->sf_target_ts - cur_time : job->decouple_target_ts - cur_time;
    ...
    uint32_t target_cpu_mhz = calculateCpuMHz(info.dispatcher_mc, remain_time);
    ...
    const sp<HWCDisplay>& display, DispatcherJob* job)
{
    ...
    if (is_dirty)
    {
        ...
        if (job->dirty_pq_mode_id)
        {
            job->pq_fence_fd = getPqDevice()->setDisplayPqMode(display->getDrmIdConnector(),
                    job->drm_id_cur_crtc, job->pq_mode_id, m_curr_present_fence_fd);
        }
