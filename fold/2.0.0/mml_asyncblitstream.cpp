...
DP_STATUS_ENUM MMLASyncBlitStream::setPQParameter(uint32_t portIndex, const DpPqParam& pqParam, uint32_t connectorID)
{
    uint32_t pq_mem_id = 0;
    MMLPQ2ndOutputInfo pq_dest;
    MMLPQHDRMetaDataInfo metadata;
    DpFW2MML_pqAll(pqParam, m_config.pq_param[portIndex], connectorID);
    //datasoace
    ...
    DpFW2MML_pqAll(pqParam, m_config.pq_param[1], connectorID);
...
void MMLASyncBlitStream::DpFW2MML_pqAll(const DpPqParam &pqParam, mml_pq_param *mmlPqParam, uint32_t connectorID)
{
    ...
    mmlPqParam->connector_id = connectorID;
}
...
void MMLASyncBlitStream::fillMMLFrameInfo(mml_frame_info* query, uint32_t srcWidth,
                                ...
                                bool isAlphaUsed, bool secure, uint32_t connectorID)
{
    ...
    DpFW2MML_pqAll(*PqParam, &pq_para, connectorID);
...
bool MMLASyncBlitStream::queryHWSupport(uint32_t srcWidth, uint32_t srcHeight,
                                ...
                                HWCLayer* layer, bool isAlphaUsed, bool secure, uint32_t connectorID)
{
    ...
    fillMMLFrameInfo(query, srcWidth, srcHeight, dstWidth, dstHeight, Orientation, srcFormat,
                     dstFormat, PqParam, srcCrop, compress, layer, isAlphaUsed, secure, connectorID);
