...
AsyncBliterHandler::~AsyncBliterHandler()
{
    ...
    // blitWorker must deconstructor before bliterNode since it will use it
    m_worker = nullptr;
...
void AsyncBliterHandler::set(const sp<HWCDisplay>& display, DispatcherJob* job)
{
    ...
    for (uint32_t i = 0; i < total_num; i++)
    {
        ...
        if (job->mdp_disp_pq)
        {
            // skip non ct and non mm layers
            if (HWC_LAYER_TYPE_FBT != hw_layer->type && HWC_LAYER_TYPE_MM != hw_layer->type) continue;
        }
        else
        {
            // skip non mm layers
        ...
        if (doBlit(hw_layer))
        {
            ...
            setPQEnhance(job->is_internal, hw_layer->priv_handle, &pq_enhance, is_game, is_camera_preview_hdr);
            ...
            config->dst_dataspace = hw_layer->mdp_output_dataspace;
            ...
            if (err == NO_ERROR)
            {
                ...
                if (is_mml)
                {
                    ...
                    WDT_BL_NODE(invalidate, hw_layer->mdp_job_id,
                        job->sequence, job->active_config, job->present_after_ts,
                        job->decouple_target_ts, job->mdp_disp_pq,
                        BliterNode::HWC_2D_BLITER_PROCESSER_MML,
                        &release_fence_fd, job->drm_id_cur_connector);
                }

                hw_layer->layer.releaseFenceFd = release_fence_fd;
                if (hw_layer->type != HWC_LAYER_TYPE_FBT)
                {
