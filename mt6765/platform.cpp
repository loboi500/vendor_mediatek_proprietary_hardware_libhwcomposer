...
// ---------------------------------------------------------------------------
Platform_MT6765::Platform_MT6765()
{
...
size_t Platform_MT6765::getLimitedExternalDisplaySize()
{
    // 4k resolution
    return 3840 * 2160;
}

bool Platform_MT6765::isUILayerValid(const sp<HWCLayer>& layer, int32_t* line)
{
...
bool Platform_MT6765::isMMLayerValid(const sp<HWCLayer>& layer, const int32_t pq_mode_id,
        int32_t* line)
{
    const int src_width = getSrcWidth(layer);
    const int src_height = getSrcHeight(layer);
